...
function boot({hash='',saved=null,delivery=null,blocked=false,search=''}={}){
 ...
 w.fetch=async (url,options={})=>{
  ...
  return {ok:true,json:async()=>url.includes('verify-checkout')?{paid:false}:{ready:true,schemas:['short-v4']}};
 };
...
(async()=>{
 ...
 try{
  ...
  assert.match(d.$('#app').textContent,/YOUR TALENT SHOULDBUY YOU FREEDOM/);
  assert.match(d.$('#app').textContent,/STOP TRADING YOUR WHOLE LIFEFOR A PAYCHECK/);
  ...
  assert.match(d.$('[data-career-sample-panel]').textContent,/FIND OUT WHAT THE JOB ACTUALLY PAID/);
  ...
  assert.match(d.$('[data-career-sample-panel]').textContent,/STOP CONFUSING SALES WITH YOUR PAY/);
  assert.equal(d.w.document.querySelectorAll('[data-career-sample-detail]').length,2);
